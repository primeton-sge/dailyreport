...
public class MailController {
    ...
    public String sendStatistics() {
        ...
        ExcelUtil.appendExcel(excelInfoList, dates, lastMonday, DateUtils.getCurrentMonday(0));
        String path = System.getProperty("user.dir") + "/template" + lastMonday.substring(0, 4)
                + lastMonday.substring(5, 7) + ".xlsx";
        File file = ExcelUtil.cloneFile(path, DateUtils.getCurrentMonday(0));
        try {
            mailService.send("上周考勤工作量统计", file, "xuning@primeton.com");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
